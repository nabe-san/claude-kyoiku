...
const CONCEPT_VOCABULARY_CIVICS = {
  ...
  '政治制度（国内）': ['議院内閣制', '二院制', '官僚制', '代表民主制', '政治的無関心', '多数決', '地方自治', '小選挙区制', '比例代表制', '政党政治', '利益団体（圧力団体）'],
...
- 文末だけでなく文頭も文の区切り（句点の直後、または段落・見出しの先頭）から始めること。ページの端で記号が始まっており、文の途中からしか引用できない場合は、【OCR参照テキスト】の「前ページ」を参照して文頭を補うこと
- 同様に、文末がこのページの端で切れており完結しない場合は、「次ページ」のOCRを参照して文末（句点）まで補うこと
- 前後ページを参照しても文として補えない場合は、その引用ブロック自体を出力しない（不完全な引用を残さない）
...
1つの引用につき概念タグは原則1個、多くても2個までにする（本1冊あたりの引用数が増えると、タグの種類も比例して増えすぎるため）。
